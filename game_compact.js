...
function initializeGame() {
    ...
    // Don't initialize needles here - let updateDisplay handle it
    // initializeNeedles();
...
function updateDisplay() {
    // Initialize needles on first update
    if (isFirstUpdate) {
        initializeNeedles();
        isFirstUpdate = false;
    }
    ...
    // TODO: Add progress gauge to HTML
...
// Track active needle animations to prevent stacking
const activeNeedleAnimations = new Map();

// Track if this is the first update
let isFirstUpdate = true;

// Debug mode for testing gauge animations
const DEBUG_GAUGES = window.location.search.includes('debug=gauges');

// Debug function to test gauge animations
function debugTestGauges() {
    console.log('🎯 Testing gauge animations...');
    const tests = [
        { stat: 'morale', value: Math.random() * 100, max: 100 },
        { stat: 'knowledge', value: Math.random() * 100, max: 100 },
        { stat: 'safety', value: Math.random() * 100, max: 100 },
        { stat: 'hours', value: Math.random() * 50, max: 50 },
        { stat: 'money', value: Math.random() * 20000, max: 20000 },
        { stat: 'fatigue', value: Math.random() * 100, max: 100 }
    ];
    
    tests.forEach(({ stat, value, max }) => {
        console.log(`Testing ${stat}: ${value.toFixed(1)}/${max}`);
        updateMiniGauge(stat, value, max);
    });
}

if (DEBUG_GAUGES) {
    window.debugTestGauges = debugTestGauges;
    console.log('🔧 Gauge debug mode enabled. Use debugTestGauges() to test animations.');
}

// Initialize needles to their starting positions
function initializeNeedles() {
    // Set initial positions for all needles without animation
    const needles = [
        { id: 'morale-mini-needle', value: gameState.stats.morale },
        { id: 'knowledge-mini-needle', value: gameState.stats.knowledge },
        { id: 'safety-mini-needle', value: gameState.stats.safety },
        { id: 'hours-mini-needle', value: gameState.stats.flightHours, max: 50 },
        { id: 'money-mini-needle', value: gameState.stats.money, max: 20000 },
        { id: 'fatigue-mini-needle', value: gameState.stats.fatigue }
    ];
    
    needles.forEach(({ id, value, max = 100 }) => {
        const needle = document.getElementById(id);
        if (needle) {
            const percentage = Math.min(100, (value / max) * 100);
            const rotation = -120 + (percentage / 100) * 240;
            // Force immediate update without transition on init
            ...
            console.log(`Initialized ${id} to rotation ${rotation}°`);
        } else {
            console.error(`Needle ${id} not found!`);
        }
    });
}

// Smooth needle animation function for mini gauges
function animateMiniNeedle(needle, fromAngle, toAngle, duration = 600) {
    if (!needle) return;
    
    // Cancel any existing animation for this needle
    const needleId = needle.id;
    if (activeNeedleAnimations.has(needleId)) {
        cancelAnimationFrame(activeNeedleAnimations.get(needleId));
        activeNeedleAnimations.delete(needleId);
    }
    ...
    function updateNeedle(currentTime) {
        ...
        // For SVG group elements, we need to use setAttribute
        needle.setAttribute('transform', `rotate(${currentAngle} 20 20)`);
        
        if (progress < 1) {
            const animationId = requestAnimationFrame(updateNeedle);
            activeNeedleAnimations.set(needleId, animationId);
        } else {
            activeNeedleAnimations.delete(needleId);
            // Ensure final position is set
            needle.setAttribute('transform', `rotate(${toAngle} 20 20)`);
        }
    }
...
function updateMiniGauge(stat, value, max = 100) {
    ...
    if (!needle) {
        console.error(`Needle not found for ${stat}`);
        return;
    }
    if (!valueEl) {
        console.error(`Value element not found for ${stat}`);
        return;
    }
    ...
    // Update needle rotation (proper gauge mapping)
    // -120° = 0% (7 o'clock position)
    // 120° = 100% (5 o'clock position)
    // Total sweep: 240 degrees
    const newRotation = -120 + (percentage / 100) * 240;
    
    // Get current rotation from CSS custom property
    const currentRotationStyle = getComputedStyle(needle).getPropertyValue('--needle-rotation');
    const oldRotation = currentRotationStyle ? parseFloat(currentRotationStyle) : -120;
    
    // Update needle rotation using CSS custom property (CSS transition will handle animation)
    if (Math.abs(newRotation - oldRotation) > 0.5) {
        if (DEBUG_GAUGES) {
            console.log(`${stat}: ${percentage.toFixed(1)}% → ${newRotation.toFixed(1)}°`);
        }
        // Use CSS custom property for reliable rotation
        needle.style.setProperty('--needle-rotation', `${newRotation}deg`);
    }
    
    // Update fill circle with status color
    if (fill) {
...
function checkForEvents() {
    ...
    // 25% chance of random event (reduced from double filtering)
    if (Math.random() < 0.25) {
        const event = selectRandomEvent(gameState);
        if (event) {
            displayEvent(event);
            return;
        }
...
function showDefaultActions() {
    ...
    if (weather.flyable) {
        ...
        if (gameState.milestones.firstSolo) {
            ...
            const dualBtn = createActionButton('Fly with CFI', 'fly_dual', 0, 
            ...
            const soloBtn = createActionButton('Fly Solo', 'fly_solo', 0, 'Solo practice flight');
            ...
            const flyBtn = createActionButton('Fly', 'fly', 0, 
...
function performAction(action) {
    ...
    switch(action) {
        ...
        case 'fly_solo':
            ...
            const flightHours = parseFloat(flightCost.hobbs);
