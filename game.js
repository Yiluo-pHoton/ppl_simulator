...
// Simplified lesson cost calculation based on phase
function calculateLessonCost() {
    // Simple phase-based costs (aircraft + instructor included)
    const phaseCosts = {
        'Ground School': 280,
        'Pre-Solo': 320,
        'Solo Training': 350,
    ...
    // Get base cost for current phase
    const baseCost = phaseCosts[gameState.phase] || 350;
    
    // Add small random variation (±10%)
    const variation = 0.9 + (Math.random() * 0.2);
    const totalCost = Math.floor(baseCost * variation);
    
    // Calculate flight hours (1.3-1.7 hours per lesson)
    const hours = 1.3 + (Math.random() * 0.4);
    
    return {
        cost: totalCost,
        hours: Math.round(hours * 10) / 10,
        aircraft: { type: 'Cessna 172', hourlyRate: 150 },
        instructor: { name: 'John Peterson', rate: 65 }
    };
}
...
function animateMilestoneElement(milestone) {
    const milestoneEl = document.getElementById(`milestone-${milestone}`);
    if (milestoneEl) {
